    ...
    def __init__(self, n_channel=100, bias=False):
        ...
        self.layers = nn.ModuleDict({
            'layer0':nn.Sequential(
                nn.ConvTranspose2d(n_channel, 512, 4, 1, 0, bias=bias),
                nn.BatchNorm2d(512),
                nn.ReLU()
            ),
            'layer1':nn.Sequential(
                nn.ConvTranspose2d(512, 512, 4, 2, 1, bias=bias),
                nn.BatchNorm2d(512),
                nn.ReLU()
            ),
...
class Discriminator(nn.Module):
    def __init__(self, bias=False):
        ...
        self.layers = nn.ModuleDict({
            'layer0':nn.Sequential(
                nn.Conv2d(3, 64, 4, 2, 1, bias=bias),
            ...
            'layer3':nn.Sequential(
                nn.Conv2d(256, 512, 4, 2, 1, bias=bias),
                nn.BatchNorm2d(512),
                nn.LeakyReLU(negative_slope=0.2)
            ),
            'layer4':nn.Sequential(
                nn.Conv2d(512, 512, 4, 2, 1, bias=bias),
                nn.BatchNorm2d(512),
                nn.LeakyReLU(negative_slope=0.2)
            ),
            'layer5':nn.Sequential(
