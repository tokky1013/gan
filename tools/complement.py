...
with torch.no_grad():
    noize = torch.zeros((100, n_channel, 1, 1), device=device)
    a = torch.randn(10, n_channel, 1, 1, device=device)
    b = torch.randn(10, n_channel, 1, 1, device=device)
    for i in range(10):
        noize[10*i:10*(i+1)] = a * i/10 + b * (1 - i/10)
    fake_image = generator(noize)
vutils.save_image(fake_image.detach(), os.path.join(fake_image_path, f'generated_images.png'), normalize=True, nrow=10)